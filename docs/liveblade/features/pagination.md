...
All you need to do is to use the `liveblade.Paginator` class. Simple as that !
...
class PostList(liveblade.Components):
    
    def render(self):
        post_list = Post.objects.all()
        posts = liveblade.Paginator(post_list, 25).without_url() # Show 25 posts per page. 
...
As you can see, in addition to limiting the number of posts shown per page, we can also use the `links()` method on the `livablade.Paginator` object to render page navigation links.

For more information on pagination using Django, check out [Django's comprehensive pagination documentation](https://docs.djangoproject.com/en/5.2/topics/pagination/).
...
If you wish to still use Liveblade's pagination utility, but disable query string tracking, you can do so using the `without_url` method on the paginator object:

```python

posts = liveblade.Paginator(post_list, 25).without_url()
```
...
class SearchPosts(liveblade.Component):
    query: str = ''

    def search(self):
        self.posts.reset_page()

    def render(self):
        post_list = Post.objects.filer(title__icontains=self.query)
        self.posts = liveblade.Paginator(post_list, 25) # Show 25 posts per page. 
...
Because both Django and Liveblade use URL query string parameters to store and track the current page number, if a single page contains multiple paginators, it's important to assign them different names.

To demonstrate the problem more clearly, consider the following `ShowClients` component:

```php
use Liveblade\WithPagination;
...
When using Liveblade's page navigation methods on a named paginator, you must provide the page name as an additional parameter:

```php
self.setPage(2, pageName: 'invoices-page');

self.resetPage(pageName: 'invoices-page');

self.nextPage(pageName: 'invoices-page');

self.previousPage(pageName: 'invoices-page');
```
...
You can use Django's `simplePaginate()` method instead of `paginate()` for added speed and simplicity.
...
For more information on simple pagination, check out [Django's "simplePaginator" documentation](https://laravel.com/docs/pagination#simple-pagination).

## Using cursor pagination

Liveblade also supports using Django's cursor pagination — a faster pagination method useful in large datasets:
...
For more information on cursor pagination, check out [Django's cursor pagination documentation](https://laravel.com/docs/pagination#cursor-pagination).

## Using Bootstrap instead of Tailwind

If you are using [Bootstrap](https://getbootstrap.com/) instead of [Tailwind](https://tailwindcss.com/) as your application's CSS framework, you can configure Liveblade to use Bootstrap styled pagination views instead of the default Tailwind views.
...
> [!info] Publishing Liveblade's configuration file
> Before customizing the pagination theme, you must first publish Liveblade's configuration file to your application's `/config` directory by running the following command:
> ```shell
> php artisan liveb-publish --config
> ```

## Modifying the default pagination views

If you want to modify Liveblade's pagination views to fit your application's style, you can do so by *publishing* them using the following command:

```shell
php artisan liveb-publish --pagination
```
...
Once the files have been published, you have complete control over them. When rendering pagination links using the paginated result's `->links()` method inside your template, Liveblade will use these files instead of its own.

## Using custom pagination views

If you wish to bypass Liveblade's pagination views entirely, you can render your own in one of two ways:
...
When rendering the pagination links, Liveblade will now look for a view at `resources/views/custom-pagination-links.blade.php`.
...
As you can see, you can use Liveblade's page navigation helpers like `self.nextPage()` directly inside your template by adding `b-click="nextPage"` to buttons:
        ...
        <nav role="navigation" aria-label="Pagination Navigation">
            <span>
                @if (paginator->onFirstPage())
                    <span>Previous</span>
                @else
                    <button b-click="previousPage" b-loading.attr="disabled" rel="prev">Previous</button>
                @endif
            </span>
            ...
            <span>
                @if (paginator->onLastPage())
                    <span>Next</span>
                @else
                    <button b-click="nextPage" b-loading.attr="disabled" rel="next">Next</button>
                @endif
            </span>
        </nav>
    @endif
