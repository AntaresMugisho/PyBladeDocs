...
By default, PyBlade Live only sends a network request when the form is submitted (or any other [action](/docs/liblade/actions) is called), not while the form is being filled out.
...
<Callout type="info">
PyBlade Live has a lot more validation features to offer. For more information, visit our dedicated documentation page on [Validation](/docs/live/features/validation).
</Callout>
...
<Callout type="info">
You can learn more about the `pb-loading` directive and other PyBlade Live directives in the [PyBlade Live Directives](/docs/live/live-directives#pb-loading) documentation.
</Callout>
...
<Callout type="info">
To learn more about the `updated()` hook and other livecycle hooks, visit the [Lifecycle hooks](/docs/live/lifecycle-hooks) documentation.
</Callout>
...
It can be helpful to extract repetitive UI elements such as these into dedicated [PyBlade components](/docs/components) to be shared across your application.
