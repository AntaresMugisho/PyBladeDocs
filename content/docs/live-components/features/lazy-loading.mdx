...
<Callout type="info" title={"Lazy requests are isolated by default"}>
...
<Callout type="warn" title={"The placeholder and the component must share the same element type"}>
