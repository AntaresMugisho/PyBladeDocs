...
You can use the same expressions supported by PyBlade's expression evaluator, including comparisons and logical operators:
...
<Callout type="info">
`@if` can be used on its own, or combined with `@elif` and `@else`. Always close the directive with `@endif`.
</Callout>
...
#### Sending a CSRF token with JavaScript

Browsers do not automatically add a CSRF token to `fetch()` requests. If your application sends JSON or another request with JavaScript, expose the token in the page and send it in the header your framework expects.
...
### Choosing an HTTP method with `@method`

Plain HTML forms can submit only `GET` and `POST`. Some frameworks support a hidden `_method` field to represent `PUT`, `PATCH`, or `DELETE`. Use `@method` only when your framework recognizes that convention.
...
PyBlade writes the hidden method field; the form itself still uses `method="POST"`.

<Callout type="info">
Some framework integrations use another way to choose an HTTP method. In that case, follow the integration's convention instead of adding `@method`.
</Callout>

### Conditional input attributes

An attribute directive adds its matching HTML attribute only when its expression is true. This keeps the decision next to the element it affects. Without parentheses, the directive is always true.

```html
<input type="text" name="username" @disabled>
```

The example above renders as `<input type="text" name="username" disabled>`.

The following directives work this way: `@checked`, `@selected`, `@disabled`, `@readonly`, `@required`, `@autofocus`, `@autocomplete`, and `@multiple`.
...
<button type="submit" @disabled(form.errors)>Save</button>
```

In this example, the checkbox stays checked when the preference is enabled, the matching option stays selected, and the button is disabled while the form has errors.

Here are a few more common examples:

```html
<input type="email" name="email" @readonly(user.is_not_admin())>
<input type="text" name="title" @required(user.is_admin())>
<input type="text" name="search" @autofocus(is_search_page)>
<input type="email" name="email" @autocomplete(preferences.autocomplete_enabled)>
<select name="labels" @multiple(user.can_choose_many)></select>
```
...
Once the same field-and-error pattern appears more than once, make a component. The component keeps your page templates short while still showing every error in the same helpful way.

```html
<!-- templates/components/text-input.html -->
<div class="flex flex-col gap-2">
    @field(field, {{ attributes.merge({"class": "form-control"}) }})
    @error(field)
        <small class="text-red-500">{{ message }}</small>
    @enderror
</div>
...
You can then use it for each field:

```html
<form method="POST" action="/contact">
    @csrf

    <pb-text-input :field="form.name" placeholder="Your name" required />
    <pb-text-input :field="form.email" placeholder="you@example.com" />
