...
The value passed to `@extends` uses **dot notation** to identify the layout file. So `layouts.base` refers to:
...
After extending a layout, the child template can provide content for the blocks defined by the parent.

In PyBlade, child templates use the `@block` directive to override a block:
...
Any content in the child template that is **not inside an `@block` directive** is automatically collected into a default slot named `slot`.
