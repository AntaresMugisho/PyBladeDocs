...
In PyBlade, displaying data inside templates is straightforward and designed to feel intuitive for developers. To display data, PyBlade uses the `{{ }}` syntax, which is both secure and simple to use.

Here's how you can display data in PyBlade templates:

## Basic variable display

To display a variable, simply wrap it within double curly braces `{{ }}`.
...
In PyBlade, you can define fallback values directly inside `{{ ... }}` using the Python `or` operator.
...
In this example, the `@` symbol will be removed by PyBlade; however, `{{ name }}` expression will remain untouched by the PyBlade Template engine, allowing it to be rendered by your JavaScript framework.
...
- **Without `@verbatim`**: The template engine would try to parse `{{ message }}` and the `@if` statement, assuming it's a PyBlade directive.
